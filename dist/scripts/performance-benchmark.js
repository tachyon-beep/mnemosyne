...
class PerformanceBenchmarkSuite {
    ...
    async initialize() {
        ...
        this.enhancedSearchEngine = new EnhancedSearchEngine(this.dbManager, this.embeddingManager, this.searchEngine);
        // Start monitoring
    ...
    async runEmbeddingBenchmarks() {
        ...
        results.push(await this.benchmarkOperation({
            ...
            operation: async () => {
                const embedding = await this.embeddingManager.generateEmbedding('This is a test sentence for embedding generation performance testing.');
        ...
        results.push(await this.benchmarkOperation({
            ...
            operation: async () => {
                const texts = Array.from({ length: 10 }, (_, i) => `Test sentence number ${i} for batch embedding generation performance evaluation.`);
    ...
    async runConcurrencyBenchmarks() {
        ...
        results.push(await this.benchmarkConcurrentOperation({
            ...
            operation: async (index) => {
                const embedding = await this.embeddingManager.generateEmbedding(`Concurrent embedding test ${index} with longer text for realistic testing.`);
                return { dimensions: embedding.length };
            },
            totalOperations: 30
        }));
    ...
    async benchmarkOperation(config) {
        const times = [];
        const errors = [];
        let successCount = 0;
        const memoryBefore = process.memoryUsage();
    ...
    async benchmarkConcurrentOperation(config) {
        const times = [];
        const errors = [];
        let successCount = 0;
        const memoryBefore = process.memoryUsage();
    ...
    async generateTestData() {
        ...
        const conversations = await Promise.all(Array.from({ length: 50 }, async (_, i) => {
    ...
    compileBenchmarkResults(results, totalDuration) {
        const passed = results.filter(r => r.passed).length;
        const failed = results.filter(r => !r.passed).length;
        const warnings = results.filter(r => !r.passed && r.averageLatency <= r.target * 1.5 // Close to target
        ).length;
