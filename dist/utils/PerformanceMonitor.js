...
export class PerformanceMonitor extends EventEmitter {
    ...
    constructor(dbManager, memoryManager, options = {}) {
        super();
     ...
     */
    async initializeDynamicThresholds() {
    ...
    getThreshold(category, metric) {
        ...
        return this.FALLBACK_THRESHOLDS[fallbackKey] || null;
    }
    /**
     ...
     */
    recordDatabaseMetric(operation, duration, resultCount = 0) {
        this.recordMetric({
            category: 'database',
            name: operation,
            value: duration,
     ...
     */
    recordSearchMetric(searchType, duration, resultCount, cacheHit = false) {
        this.recordMetric({
            category: 'search',
     ...
     */
    recordEmbeddingMetric(operation, duration, batchSize = 1) {
     ...
     */
    getPerformanceReport(durationMs = 300000) {
        const now = Date.now();
    ...
    cleanupOldMetrics() {
        ...
        // Clean up old alerts
        this.alertHistory = this.alertHistory.filter(a => a.triggeredAt > cutoff || (a.resolvedAt && a.resolvedAt > cutoff));
    }
    getMetricsInRange(startTime, endTime) {
        const result = [];
        for (const metrics of this.metrics.values()) {
            result.push(...metrics.filter(m => m.timestamp >= startTime && m.timestamp <= endTime));
        }
        return result;
    }
    getMetricsByCategory(metrics, category) {
        return metrics.filter(m => m.category === category);
    }
    getMetricsByName(metrics, name) {
        return metrics.filter(m => m.name === name);
    }
    sumMetricValues(metrics, category, nameFilter) {
        return metrics
            .filter(m => m.category === category && (!nameFilter || m.name.includes(nameFilter)))
            .length; // Count of metrics, not sum of values
    }
    aggregateMetrics(metrics, category) {
    ...
    generateRecommendations(metrics) {
        ...
        const highMemoryUsage = memoryMetrics.filter(m => m.name === 'heap_usage_percent' && m.value > 0.7);
        ...
        const slowSearches = searchMetrics.filter(m => m.name.includes('duration') && m.value > this.FALLBACK_THRESHOLDS.SEARCH_RESPONSE_TIME);
     ...
     */
    getEnhancedPerformanceReport(durationMs = 300000) {
