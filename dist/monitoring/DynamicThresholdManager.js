...
export class DynamicThresholdManager extends EventEmitter {
    ...
    isInitialized = false;
    constructor(configPath = './data/dynamic-thresholds.json') {
        super();
     ...
     */
    async profileSystemCapabilities() {
     ...
     */
    updateBaseline(metric, category, value) {
        ...
        if (!baseline) {
            ...
            baseline.standardDeviation = Math.sqrt(baseline.standardDeviation * baseline.standardDeviation * (1 - alpha) + variance * alpha);
     ...
     */
    adaptThreshold(metric, category, baseline) {
    ...
    async optimizeThresholds() {
        ...
        for (const [thresholdId, performance] of thresholdPerformance.entries()) {
            ...
            if (Math.abs(optimalValue - current.currentValue) / current.currentValue > 0.1) {
                ...
                reasoning.push(`${thresholdId}: ${current.currentValue.toFixed(2)} → ${optimalValue.toFixed(2)} ` +
     ...
     */
    findOptimalThreshold(thresholdId, performance) {
     ...
     */
    async applyOptimizationRecommendations(result) {
     ...
     */
    recordTrainingData(performanceMetrics, alertCount, falsePositiveCount = 0, missedIssueCount = 0) {
        const context = this.getCurrentSystemContext();
    ...
    getThresholdReport() {
        ...
        const recentlyAdjusted = currentThresholds.filter(t => Date.now() - t.lastAdjustment < 60 * 60 * 1000 // Last hour
        );
