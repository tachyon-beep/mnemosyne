...
export class EnhancedPerformanceMonitor extends EventEmitter {
    ...
    constructor(dbManager, memoryManager, configPath = './data/enhanced-monitoring.json') {
        super();
     ...
     */
    recordEnhancedMetric(category, name, value, unit, tags) {
     ...
     */
    async applySystemOptimizedSettings() {
     ...
     */
    calculateTrend(category, metric) {
        ...
        if (change > 0.1)
            return 'degrading'; // 10% degradation
        return 'stable';
    }
    /**
    ...
    async collectEnhancedMetrics() {
        ...
        try {
            ...
            this.recordEnhancedMetric('database', 'cache_size', cacheSize, 'count');
        }
        catch (error) {
     ...
     */
    async processPotentialAlert(metric, threshold, thresholdId) {
        ...
        const contextualAlert = await this.contextualAlerts.processAlert(metric.name, metric.category, metric.value, threshold, severity, message);
        if (contextualAlert) {
            ...
            this.dynamicThresholds.recordTrainingData({ [thresholdId]: metric.value }, 1, // Alert count
     ...
     */
    generateSystemRecommendations(components, metrics) {
    ...
    // Helper methods
    getRecentMetrics(category, name, windowMs) {
        const key = `${category}:${name}`;
        const metrics = this.metrics.get(key) || [];
        const cutoff = Date.now() - windowMs;
        return metrics.filter(m => m.timestamp > cutoff);
    }
    getTotalRecentOperations() {
...
//# sourceMappingURL=EnhancedPerformanceMonitor.js.map