...
export class PatternDetectionService extends BaseRepository {
     ...
     */
    async detectUnresolvedActions(options = {}) {
        const { conversationId, daysSince = 7, minConfidence = 0.6, limit = 50 } = options;
        ...
        params.push(limit);
        const messages = this.executeStatementAll('find_potential_commitments', `SELECT m.*, c.title as conversation_title
       FROM messages m
       ...
       LIMIT ?`, params);
        ...
        for (const row of messages) {
            ...
            const hasFollowUp = await this.hasFollowUpMention(message.conversationId, message.createdAt, commitmentMatch.commitmentText);
            const daysSinceCommitment = Math.floor((Date.now() - message.createdAt) / (24 * 60 * 60 * 1000));
    ...
    async findRecurringQuestions(options = {}) {
        ...
        const messages = this.executeStatementAll('find_question_messages', `SELECT m.*
       FROM messages m
       ${whereClause}
    ...
    async identifyKnowledgeGaps(options = {}) {
        const { conversationId, minGapRatio = 1.5, limit = 15 } = options;
        ...
        for (const [topic, data] of topicMap.entries()) {
            const answers = await this.findAnswersForTopic(topic, data.questions, conversationId);
     ...
     */
    async trackCommitments(options = {}) {
        ...
        const messages = this.executeStatementAll('find_commitment_messages', `SELECT m.*
       FROM messages m
       ${whereClause}
       ORDER BY m.created_at DESC
       LIMIT ?`, params);
        ...
        for (const row of messages) {
            ...
            const followUps = await this.findFollowUpMessages(message.conversationId, message.createdAt, commitmentMatch.commitmentText);
            const daysSinceCommitment = Math.floor((Date.now() - message.createdAt) / (24 * 60 * 60 * 1000));
            const status = this.determineCommitmentStatus(commitmentMatch, followUps, daysSinceCommitment);
     ...
     */
    calculateCommitmentConfidence(text, pattern) {
        let confidence = 0.6; // Base confidence
        // Boost confidence for specific patterns
        if (pattern.source.includes("I'll"))
            confidence += 0.2;
        if (pattern.source.includes("need to"))
            confidence += 0.1;
        if (text.includes("tomorrow") || text.includes("next week"))
            confidence += 0.1;
     ...
     */
    async hasFollowUpMention(conversationId, afterTimestamp, commitmentText) {
     ...
     */
    isQuestion(content) {
        return PatternDetectionService.QUESTION_PATTERNS.some(pattern => pattern.test(content.trim()));
    }
    /**
    ...
    async findAnswersForTopic(topic, questions, conversationId) {
        ...
        const rows = this.executeStatementAll('find_topic_answers', `SELECT m.*
       FROM messages_fts
       JOIN messages m ON m.rowid = messages_fts.rowid
       ${whereClause}
     ...
     */
    looksLikeAnswer(content) {
        return PatternDetectionService.ANSWER_INDICATORS.some(pattern => pattern.test(content.trim())) || content.length > 50; // Longer responses more likely to be answers
    }
    /**
     * Calculate confidence for topic identification
     */
    calculateTopicConfidence(topic, questions) {
     ...
     */
    determineCommitmentStatus(commitment, followUps, daysSince) {
        ...
        const hasResolution = followUps.some(msg => resolvedIndicators.some(indicator => msg.content.toLowerCase().includes(indicator)));
        return hasResolution ? 'resolved' : 'mentioned';
    }
    /**
     * Classify the type of commitment
     */
    classifyCommitmentType(commitmentText) {
        const text = commitmentText.toLowerCase();
        if (text.includes('check'))
            return 'check';
        if (text.includes('follow') || text.includes('get back'))
            return 'follow_up';
        if (text.includes('update'))
            return 'update';
        if (text.includes('investigate') || text.includes('look into'))
            return 'investigate';
        if (/(?:by|before|after)\s+(?:tomorrow|friday|next week|end of)/i.test(text))
            return 'temporal';
     ...
     */
    extractTimeframe(commitmentText) {
        const text = commitmentText.toLowerCase();
