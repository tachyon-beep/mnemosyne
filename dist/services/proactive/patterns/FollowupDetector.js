...
export class FollowupDetector extends BaseRepository {
     ...
     */
    static ACKNOWLEDGMENT_PATTERNS = [
     ...
     */
    static DEFAULT_STALENESS_CONFIG = {
        defaultStaleDays: 7,
        urgencyThresholds: {
            low: 14,
            normal: 7,
     ...
     */
    async detectCommitmentLanguage(messages, options = {}) {
        const { minConfidence = 0.6, includeEntities = true, conversationContext = true } = options;
        const commitments = [];
        for (const message of messages) {
            const detectedCommitments = await this.analyzeMessageForCommitments(message, { minConfidence, includeEntities, conversationContext });
     ...
     */
    async trackTemporalPromises(conversationId, options = {}) {
        ...
        const messages = this.executeStatementAll('find_temporal_messages', `SELECT m.* FROM messages m ${whereClause} ORDER BY m.created_at DESC`, params);
        const temporalCommitments = [];
     ...
     */
    async identifyStaleActions(options = {}) {
        ...
        const activeCommitments = allCommitments.filter(c => ['pending', 'acknowledged', 'in_progress'].includes(c.status));
        for (const commitment of activeCommitments) {
            const stalenessThreshold = this.calculateStalenessThreshold(commitment, config);
            const daysSinceCommitment = Math.floor((Date.now() - commitment.message.createdAt) / (24 * 60 * 60 * 1000));
            if (daysSinceCommitment >= stalenessThreshold) {
                ...
                const relatedEntities = options.includeEntityContext
                    ...
                    : commitment.entities;
                const suggestedActions = await this.generateFollowupSuggestions(commitment, relatedEntities, staleness);
                staleActions.push({
                    commitment,
        ...
        return staleActions.sort((a, b) => this.getStalenessScore(b.stalenessLevel) - this.getStalenessScore(a.stalenessLevel));
    }
    /**
     * Generate follow-up suggestions based on context
     */
    async suggestFollowups(commitment, context = {}) {
        const suggestions = [];
        ...
        if (context.entityRelationships && context.relatedEntities) {
            const entitySuggestions = await this.generateEntityBasedSuggestions(commitment, context.entityRelationships, context.relatedEntities);
     ...
     */
    async analyzeMessageForCommitments(message, options) {
        const commitments = [];
        ...
        for (const [type, patterns] of Object.entries(FollowupDetector.COMMITMENT_PATTERNS)) {
            for (const pattern of patterns) {
                ...
                if (match) {
                    const commitment = await this.createCommitmentFromMatch(message, type, pattern, match, options);
     ...
     */
    async createCommitmentFromMatch(message, type, pattern, match, options) {
     ...
     */
    calculateCommitmentConfidence(text, pattern, type) {
        ...
        if (pattern.source.includes("I'll"))
            confidence += 0.15;
        if (pattern.source.includes("by|before|after"))
            confidence += 0.2;
        ...
        if (text.length > 20 && text.length < 150)
            confidence += 0.05;
        if (/\b(?:check|verify|confirm|update|review)\b/i.test(text))
            confidence += 0.05;
    ...
    async getConversationContext(conversationId) {
        const conversation = this.executeStatement('get_conversation_context', 'SELECT id, title FROM conversations WHERE id = ?', [conversationId]);
        // Count unique participants (roles)
        const { count } = this.executeStatement('count_participants', 'SELECT COUNT(DISTINCT role) as count FROM messages WHERE conversation_id = ?', [conversationId]);
        return {
            conversationId,
     ...
     */
    async findFollowUpMessages(conversationId, afterTimestamp, commitmentText) {
        const keywords = this.extractKeywords(commitmentText);
        if (keywords.length === 0)
            return [];
        const rows = this.executeStatementAll('find_followup_messages', `SELECT m.*
       FROM messages_fts
       JOIN messages m ON m.rowid = messages_fts.rowid
       WHERE m.conversation_id = ? 
         AND m.created_at > ?
         AND messages_fts MATCH ?
       ORDER BY m.created_at ASC`, [conversationId, afterTimestamp, keywords.join(' OR ')]);
        return rows.map(row => this.mapRowToMessage(row));
    }
    /**
     * Determine commitment status based on follow-ups
     */
    determineCommitmentStatus(followUps, daysSince, urgency) {
        ...
        const hasCompletion = followUps.some(msg => FollowupDetector.COMPLETION_PATTERNS.some(pattern => pattern.test(msg.content)));
        if (hasCompletion)
            return 'completed';
        // Check for acknowledgment or progress
        const hasAcknowledgment = followUps.some(msg => FollowupDetector.ACKNOWLEDGMENT_PATTERNS.some(pattern => pattern.test(msg.content)));
    ...
    extractTimeframe(text) {
        const lowerText = text.toLowerCase();
        if (lowerText.includes('today') || lowerText.includes('immediately'))
            return 0;
        ...
        if (lowerText.includes('next week'))
            return 14;
        if (lowerText.includes('this month') || lowerText.includes('end of month'))
            return 30;
    ...
    async getAllCommitments(conversationId) {
        ...
        const messages = this.executeStatementAll('get_all_messages_for_commitments', `SELECT m.* FROM messages m ${whereClause} ORDER BY m.created_at DESC`, params);
        const commitments = [];
        for (const row of messages) {
            const message = this.mapRowToMessage(row);
     ...
     */
    calculateStalenessThreshold(commitment, config) {
     ...
     */
    calculateStalenessLevel(daysSinceCommitment, threshold) {
     ...
     */
    generateStalenessReason(commitment, daysSinceCommitment, threshold) {
     ...
     */
    async getRelatedEntities(commitment) {
     ...
     */
    async generateEntityBasedSuggestions(commitment, relationships, entities) {
        const suggestions = [];
        // Find relevant relationships for escalation suggestions
        for (const entity of entities) {
            const entityRelationships = relationships.filter(r => r.sourceEntityId === entity.id || r.targetEntityId === entity.id);
...
//# sourceMappingURL=FollowupDetector.js.map