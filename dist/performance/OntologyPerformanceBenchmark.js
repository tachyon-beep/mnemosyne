...
export class OntologyPerformanceBenchmark {
    ...
    async runWarmUp() {
        ...
        for (let i = 0; i < this.config.warmupRuns; i++) {
            ...
            await this.databaseManager.executeOptimized('SELECT COUNT(*) FROM entities', []);
            await this.databaseManager.executeOptimized('SELECT * FROM entities LIMIT 10', []);
    ...
    async benchmarkCurrentApproach(scale) {
        ...
        await this.benchmarkOperation('entity_lookup_current', 'current', scale, async () => {
            ...
            for (let i = 0; i < BENCHMARK_CONSTANTS.ENTITY_LOOKUP_ITERATIONS; i++) {
                await this.databaseManager.executeOptimized('SELECT * FROM entities WHERE normalized_name = ? LIMIT 1', [`test_entity_${i % BENCHMARK_CONSTANTS.ENTITY_SAMPLE_SIZE}`]);
            }
        ...
        await this.benchmarkOperation('relationship_query_current', 'current', scale, async () => {
            const startTime = performance.now();
     ...
     */
    async benchmarkFormalOntologyApproach(scale) {
        const startMemory = process.memoryUsage().heapUsed / BENCHMARK_CONSTANTS.BYTES_TO_MB;
        // Entity lookup with type hierarchy validation
        await this.benchmarkOperation('entity_lookup_formal', 'formal', scale, async () => {
            const startTime = performance.now();
            for (let i = 0; i < BENCHMARK_CONSTANTS.ENTITY_LOOKUP_ITERATIONS; i++) {
        ...
        await this.benchmarkOperation('graph_traversal_formal', 'formal', scale, async () => {
            const startTime = performance.now();
     ...
     */
    async benchmarkEnhancedPragmaticApproach(scale) {
        const startMemory = process.memoryUsage().heapUsed / BENCHMARK_CONSTANTS.BYTES_TO_MB;
        // Entity lookup with semantic categories
        await this.benchmarkOperation('entity_lookup_enhanced', 'enhanced', scale, async () => {
            const startTime = performance.now();
            for (let i = 0; i < BENCHMARK_CONSTANTS.ENTITY_LOOKUP_ITERATIONS; i++) {
                await this.databaseManager.executeOptimized(`
            SELECT e.*, 
    ...
    async benchmarkConcurrency() {
        ...
        for (const concurrency of concurrentOperations) {
            ...
            const startMemory = process.memoryUsage().heapUsed / BENCHMARK_CONSTANTS.BYTES_TO_MB;
            try {
                const operations = Array.from({ length: concurrency }, (_, i) => this.simulateMCPToolOperation(i));
     ...
     */
    async simulateMCPToolOperation(operationId) {
     ...
     */
    async simulateTypeHierarchyValidation() {
     ...
     */
    async simulateDomainRangeValidation() {
     ...
     */
    async simulateOntologyValidation() {
     ...
     */
    async benchmarkOperation(operationName, approach, scale, operation) {
        const iterations = 5;
        const times = [];
        let errorCount = 0;
    ...
    exportResults() {
        ...
        const csvRows = this.results.map(r => `${r.operation},${r.approach},${r.scale},${r.executionTimeMs},${r.memoryUsageMB},${r.queriesExecuted},${r.cacheHitRate},${r.errorCount}`).join('\n');
        return csvHeader + csvRows;
    }
}
//# sourceMappingURL=OntologyPerformanceBenchmark.js.map