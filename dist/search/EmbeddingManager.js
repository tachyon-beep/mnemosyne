...
export class EmbeddingManager {
    ...
    performanceMetrics = { totalEmbeddings: 0, totalTime: 0, averageTime: 0 };
    circuitBreaker;
    operationLock;
    memoryManager;
    ...
    async initialize() {
        ...
        try {
            ...
            const pipelineResult = await pipeline('feature-extraction', this.config.modelName, {
     ...
     */
    updateProgress(step, progress, complete = false) {
        this.loadingProgress = {
            step,
    ...
    async generateEmbedding(text) {
        ...
        try {
            ...
            if (processingTime > this.config.performanceTarget) {
                console.warn(`Embedding generation took ${processingTime}ms ` +
        ...
        try {
            ...
            for (let i = 0; i < uncachedTexts.length; i += batchSize) {
                ...
                try {
                    ...
                    // Fall back to individual processing
                    batchEmbeddings = await Promise.all(batch.map(text => this.generateEmbedding(text)));
                }
            ...
            console.log(`Generated ${uncachedTexts.length} embeddings in ${processingTime}ms ` +
     ...
     */
    async storeEmbedding(messageId, embedding) {
     ...
     */
    async processUnembeddedMessages(batchSize = 100) {
        ...
        LIMIT ?
      `);
                const messages = stmt.all(batchSize);
     ...
     */
    async findSimilarMessages(queryEmbedding, options = {}) {
        const db = this.dbManager.getConnection();
        const { limit = 20, threshold = 0.7, conversationId, excludeMessageIds = [] } = options;
        try {
            // Input validation
      ...
      `;
            ...
            while (hasMore && similarities.length < limit * 2) { // Process 2x limit for better results
                ...
                const messages = db.prepare(chunkQuery).all(...params);
                if (messages.length === 0) {
                    hasMore = false;
                    break;
                }
    ...
    async getEmbeddingStats() {
        ...
        const embeddedMessages = db.prepare('SELECT COUNT(*) as count FROM messages WHERE embedding IS NOT NULL').get();
    ...
    async loadConfiguration() {
        const db = this.dbManager.getConnection();
        try {
            const configs = db.prepare('SELECT key, value FROM search_config').all();
     ...
     */
    async generateEmbeddingWithFallback(text, maxRetries = 2) {
