...
export class DecisionTrackingRepository extends AnalyticsRepository {
    ...
    `;
        ...
        try {
            ...
            this.handleConstraintError(error, 'decision');
        }
    }
    /**
     * Update decision outcome
     */
    async updateOutcome(outcome) {
        const now = this.getCurrentTimestamp();
        const sql = `
     ...
     */
    async markReversed(decisionId, reason) {
        const sql = `
      UPDATE decision_tracking 
      SET 
     ...
     */
    async getDecisions(status, timeRange, options) {
        ...
        const params = { ...validTimeRange, limit, offset };
        if (status) {
    ...
    `;
        const results = this.executeStatementAll('get_decisions', sql, params);
        return results.map(row => this.mapRowToDecision(row));
    }
    /**
     * Get decision analysis summary
     */
    async getDecisionAnalysis(timeRange) {
        const validTimeRange = this.validateTimeRange(timeRange);
        const sql = `
    ...
    `;
        const result = this.executeStatement('decision_analysis', sql, validTimeRange);
     ...
     */
    async getDecisionPatterns(timeRange) {
        const validTimeRange = this.validateTimeRange(timeRange);
        const sql = `
    ...
    `;
        const results = this.executeStatementAll('decision_patterns', sql, validTimeRange);
     ...
     */
    async getDecisionsNeedingFollowUp(daysOld = 30) {
      ...
      LIMIT 20
    `;
        const results = this.executeStatementAll('decisions_needing_followup', sql, { cutoffTime });
        return results.map(row => this.mapRowToDecision(row));
    }
    /**
     * Get decision by ID
    ...
    async getDecision(decisionId) {
      ...
      WHERE id = @decisionId
    `;
        const result = this.executeStatement('get_decision', sql, { decisionId });
     ...
     */
    async getTopSuccessFactors(timeRange) {
        const sql = `
      ...
      LIMIT 10
    `;
        const results = this.executeStatementAll('top_success_factors', sql, timeRange);
        return results.map(row => ({
            factor: row.factor,
            frequency: row.frequency,
     ...
     * Get common pitfalls
     */
    async getCommonPitfalls(timeRange) {
        const sql = `
      ...
      LIMIT 10
    `;
        const results = this.executeStatementAll('common_pitfalls', sql, timeRange);
        return results.map(row => ({
            pitfall: row.pitfall,
            frequency: row.frequency,
     ...
     */
    async batchSaveDecisions(decisionInputs, options = {}) {
        ...
        try {
            if (conflictResolution === 'UPDATE') {
                ...
                const result = this.batchUpsert('decision_tracking', dbRecords, ['decision_summary', 'decision_made_at'], // Unique constraint
                {
                    batchSize,
                    onProgress
                });
                ...
                const result = this.batchInsert('decision_tracking', dbRecords, {
                    batchSize,
     ...
     */
    async batchUpdateOutcomes(outcomes, options = {}) {
     ...
     */
    async batchMarkReversed(reversals, options = {}) {
     ...
     */
    async batchDeleteDecisions(decisionIds, options = {}) {
        ...
        `;
                const result = this.executeStatementRun(`batch_delete_decisions_${i}`, sql, params);
     ...
     */
    async batchTrackDecisions(conversationDecisions, options = {}) {
     ...
     */
    estimateDecisionClarity(decision) {
     ...
     */
    estimateConfidenceLevel(decision) {
        const content = (decision.content || decision.summary || '').toLowerCase();
        let confidence = 50;
     ...
     */
    estimateInformationCompleteness(decision, metadata) {
        let completeness = 40;
        const content = (decision.content || decision.summary || '').toLowerCase();
    ...
    extractDecisionTags(decision) {
        ...
        const tags = [];
        // Category tags
        if (content.includes('strategic') || content.includes('strategy'))
            tags.push('strategic');
        if (content.includes('technical') || content.includes('technology'))
            tags.push('technical');
        if (content.includes('business') || content.includes('commercial'))
            tags.push('business');
        if (content.includes('personal') || content.includes('individual'))
            tags.push('personal');
        if (content.includes('financial') || content.includes('budget') || content.includes('cost'))
            tags.push('financial');
     ...
     */
    assessDecisionPriority(decision) {
        const content = (decision.content || decision.summary || '').toLowerCase();
...
//# sourceMappingURL=DecisionTrackingRepository.js.map