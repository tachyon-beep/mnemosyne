...
export class IndexUsageMonitor {
    ...
    async startMonitoring(intervalMinutes = 15) {
        ...
        this.monitoringInterval = setInterval(async () => {
            try {
                ...
                this.createPerformanceAlert('critical', 'monitoring_error', 'Index monitoring encountered an error', { error: error instanceof Error ? error.message : String(error) });
            }
    ...
    async analyzeQueryPlan(sql, params = []) {
        ...
        try {
            ...
            const rawPlan = planStmt.all(...params);
     ...
     */
    async getUnusedIndexes(minDaysUnused = 30) {
        const unusedIndexes = [];
     ...
     */
    async generateIndexOptimizations() {
     ...
     */
    async getWritePerformanceMetrics() {
    ...
    async performComprehensiveAnalysis() {
        ...
        if (slowQueries.length > 0) {
            this.createPerformanceAlert('high', 'slow_query', `${slowQueries.length} slow queries detected`, { slowQueries: slowQueries.slice(0, 5) });
        }
        // Check for unused indexes
        const unusedIndexes = await this.getUnusedIndexes();
        if (unusedIndexes.length > 0) {
            this.createPerformanceAlert('medium', 'unused_index', `${unusedIndexes.length} unused indexes found`, { indexes: unusedIndexes });
        }
    ...
    parseQueryPlan(queryId, sql, params, rawPlan, executionTime) {
        const planSteps = [];
        const indexesUsed = [];
        const tableScans = [];
        const recommendations = [];
    ...
    async updateIndexUsageStats(analysis) {
      ...
      `);
            ...
            stmt.run(indexName, tableName, timestamp, analysis.executionTime, timestamp, timestamp, analysis.executionTime, timestamp);
        }
        ...
        ON CONFLICT(index_name) DO UPDATE SET
          ...
          updated_at = ?
      `);
            stmt.run(`table_scan_${tableName}`, tableName, timestamp, analysis.executionTime, timestamp, timestamp, analysis.executionTime, timestamp);
        }
    }
    async calculateIndexEffectiveness(indexName, tableName) {
    ...
    createPerformanceAlert(severity, type, message, details = {}) {
        const alert = {
    ...
    async checkPerformanceDegradation() {
        ...
        if (recentQueries.length > 10) {
            this.createPerformanceAlert('high', 'index_degradation', 'Performance degradation detected - increasing slow query count', { recentSlowQueries: recentQueries.length });
        }
        ...
        for (const [tableName, metrics] of writeMetrics) {
            if (metrics.writeThroughputImpact > 0.5) {
                this.createPerformanceAlert('medium', 'write_impact', `High write performance impact on ${tableName}`, { impact: metrics.writeThroughputImpact, indexCount: metrics.indexCount });
            }
        }
    }
    ...
    calculateMaintenanceCost(indexName, sizeBytes, usageCount) {
        // Cost based on size and usage frequency
        return (sizeBytes / 1024) * (usageCount / 1000);
    }
    async isIndexReferencedInConstraints(indexName) {
    ...
    async storeQueryPlanAnalysis(analysis) {
        const stmt = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(analysis.queryId, analysis.sql, analysis.executionTime, JSON.stringify(analysis.indexesUsed), JSON.stringify(analysis.tableScans), JSON.stringify(analysis.recommendations), JSON.stringify(analysis.performanceIssues));
    }
    async logIndexAction(recommendation, status, executionTime, errorMessage) {
        const stmt = this.db.prepare(`
      INSERT INTO index_optimization_log 
      (index_name, table_name, action_type, sql_executed, status, execution_time, error_message)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(recommendation.indexName, recommendation.tableName, recommendation.type, recommendation.sql, status, executionTime, errorMessage || null);
    }
...
//# sourceMappingURL=IndexUsageMonitor.js.map