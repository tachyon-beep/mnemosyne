 ...
 */
import { ConversationAnalyticsRepository, ProductivityPatternsRepository, KnowledgeGapsRepository, DecisionTrackingRepository } from '../repositories/index.js';
...
export class AnalyticsEngine {
     ...
     */
    async generateReport(timeRange, format = 'summary') {
     ...
     */
    async processNeedingAnalysis() {
        if (!this.config.enableIncrementalProcessing) {
            return 0;
        }
        const startTime = Date.now();
        const conversationIds = await this.conversationAnalytics.getConversationsNeedingAnalysis(this.config.batchProcessingSize);
     ...
     */
    async getConversationMetrics(timeRange) {
     ...
     */
    async getProductivityInsights(timeRange) {
     ...
     */
    async getKnowledgeGapMetrics(timeRange) {
     ...
     */
    async getDecisionMetrics(timeRange) {
     ...
     */
    async analyzeProductivityPatterns(timeRange) {
        const normalizedRange = this.validateTimeRange(timeRange);
        // Get conversations in time range for productivity analysis
        const conversationResult = await this.conversations.findByDateRange(normalizedRange.start, normalizedRange.end, this.config.batchProcessingSize, 0);
        ...
        for (let i = 0; i < conversations.length; i += batchSize) {
            ...
            await Promise.all(batch.map(async (conversation) => {
                ...
                try {
                    ...
                    const productivityMetrics = await this.productivityAnalyzer.analyzeConversationProductivity(conversation, messages);
     ...
     */
    async analyzeKnowledgeGaps(timeRange) {
        const normalizedRange = this.validateTimeRange(timeRange);
        const conversationResult = await this.conversations.findByDateRange(normalizedRange.start, normalizedRange.end, this.config.batchProcessingSize, 0);
        const conversations = conversationResult.data;
        let processed = 0;
     ...
     */
    estimateSessionDuration(messages) {
     ...
     */
    async processKnowledgeGaps(knowledgeGaps, conversationId) {
     ...
     */
    async fallbackProcessKnowledgeGaps(knowledgeGaps, conversationId) {
     ...
     */
    async processDecisions(decisions, conversationId) {
     ...
     */
    async fallbackProcessDecisions(decisions, conversationId) {
     ...
     */
    async calculateProductivityTrend(timeRange) {
        try {
            // Get conversations from the time range
            const conversationResult = await this.conversations.findByDateRange(timeRange.start, timeRange.end, 1000, 0);
            const conversations = conversationResult.data;
            if (conversations.length < 2) {
                return 0; // Not enough data for trend
            }
            // Get analytics for these conversations
            const analytics = await Promise.all(conversations.map(conv => this.conversationAnalytics.getConversationAnalytics(conv.id)));
     ...
     */
    async batchProcessConversations(conversationIds, options = {}) {
        const { batchSize = 50, analysisTypes = ['analytics', 'productivity', 'knowledge-gaps', 'decisions'], onProgress, maxProcessingTimeMs = this.config.maxProcessingTimeMs } = options;
        const startTime = Date.now();
        let processed = 0;
        let failed = 0;
        ...
        try {
            ...
            if (analysisTypes.includes('analytics') && analysisResults.analytics.length > 0) {
                ...
                const analyticsResult = await this.conversationAnalytics.batchSaveAnalytics(analysisResults.analytics, {
                    batchSize,
            ...
            if (analysisTypes.includes('productivity') && analysisResults.productivity.length > 0) {
                ...
                const productivityResult = await this.productivityPatterns.bulkAnalyzeProductivityPatterns(conversations, 'day', // Default to daily patterns
                {
                    batchSize,
            ...
            if (analysisTypes.includes('knowledge-gaps') && analysisResults.knowledgeGaps.length > 0) {
                ...
                const knowledgeGapResult = await this.knowledgeGaps.batchProcessGapsFromConversations(analysisResults.knowledgeGaps, {
                    batchSize,
                    deduplication: true,
            ...
            if (analysisTypes.includes('decisions') && analysisResults.decisions.length > 0) {
                ...
                const decisionResult = await this.decisionTracking.batchTrackDecisions(analysisResults.decisions, {
                    batchSize,
     ...
     */
    async loadConversationsBatch(conversationIds) {
        const conversations = [];
     ...
     */
    async batchAnalyzeConversations(conversations, options = {}) {
