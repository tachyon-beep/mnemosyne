...
export class ConversationFlowAnalyzer {
    ...
    TRANSITION_TIME_THRESHOLD = 300000; // 5 minutes
    /**
     * Analyze complete conversation flow
     */
    async analyzeFlow(conversation, messages) {
        try {
     ...
     */
    createDefaultFlowMetrics(conversationId) {
        return {
            conversationId,
            analyzedAt: Date.now(),
            topics: [],
    ...
    safeBuildTransitionGraph(topics, messages) {
        try {
    ...
    safeCalculateDepthScore(messages, topics) {
        try {
    ...
    safeAnalyzeCircularity(topics, transitions) {
        try {
    ...
    safeCalculateCoherence(topics, transitions) {
        try {
    ...
    safeCalculateProgression(topics, transitions) {
        try {
    ...
    safeAnalyzeTimingPatterns(messages, topics, transitions) {
        try {
    ...
    safeCalculateEngagementMetrics(messages) {
        try {
    ...
    safeCalculateContentMetrics(messages) {
        try {
    ...
    safeDetectResolutionTime(messages) {
        try {
    ...
    safeExtractMessageTopics(message) {
        try {
     ...
     */
    buildTransitionGraph(topics, messages) {
        try {
    ...
    safeClassifyTransition(fromTopic, toTopic, timeGap, messages) {
        try {
    ...
    safeCalculateTransitionConfidence(fromTopic, toTopic, timeGap) {
        try {
     ...
     */
    analyzeCircularity(topics, transitions) {
        try {
    ...
    safeFindStronglyConnectedComponents(graph) {
        try {
     ...
     */
    calculateDepthScore(messages, topics) {
        try {
            if (!messages || messages.length === 0) {
                return 0;
            }
    ...
    safeCalculateVocabularyRichness(messages) {
        try {
    ...
    safeCalculateTopicDepth(topics) {
        try {
    ...
    safeCalculateQuestionComplexity(messages) {
        try {
     ...
     */
    calculateCoherence(topics, transitions) {
        try {
     ...
     */
    calculateProgression(topics, transitions) {
        try {
    ...
    safeCalculateTopicEvolution(topics) {
        try {
     ...
     */
    analyzeTimingPatterns(messages, topics, transitions) {
        if (topics.length === 0) {
            return {
                averageTopicDuration: 0,
                fastestTransition: 0,
    ...
    calculateEngagementMetrics(messages) {
        ...
        const insightCount = messages.filter(m => insightIndicators.some(indicator => m.content.toLowerCase().includes(indicator))).length;
     ...
     */
    calculateContentMetrics(messages) {
        if (messages.length === 0) {
            return {
                averageLength: 0,
                vocabularyRichness: 0
    ...
    generateTopicId(normalizedContent, messageId) {
        return `${normalizedContent.replace(/\s+/g, '_')}_${messageId.slice(-8)}`;
    }
    extractQuestionTopic(content) {
    ...
    calculateTransitionConfidence(fromTopic, toTopic, timeGap) {
        const similarity = this.calculateTopicSimilarity(fromTopic, toTopic);
    ...
    calculateVocabularyRichness(messages) {
        try {
            if (!messages || messages.length === 0) {
                return 0;
            }
    ...
    calculateQuestionComplexity(messages) {
    ...
    calculateTopicEvolution(topics) {
    ...
    detectResolutionTime(messages) {
        ...
        for (let i = messages.length - 1; i >= 0; i--) {
            ...
            const hasResolution = resolutionIndicators.some(indicator => message.content.toLowerCase().includes(indicator));
            if (hasResolution) {
...
//# sourceMappingURL=ConversationFlowAnalyzer.js.map