 ...
 */
export class SuggestRelevantContextTool extends BaseTool {
    contextDetector;
    ...
    async executeImpl(input, context) {
        ...
        const suggestions = await wrapDatabaseOperation(async () => await this.knowledgeSynthesizer.suggestRelevantContext(entityIds, input.currentConversationId || '', input.limit), 'Failed to generate context suggestions');
        // Filter suggestions by requested types and relevance
        const filteredSuggestions = suggestions
            .filter(suggestion => input.contextTypes.includes(suggestion.type) &&
        ...
        if (input.currentConversationId) {
            contextWindow = await wrapDatabaseOperation(async () => await this.contextDetector.analyzeContextWindow(input.currentConversationId, {
        ...
        if (input.includeExperts && entityIds.length > 0) {
            expertRecommendations = await wrapDatabaseOperation(async () => await this.knowledgeSynthesizer.recommendExperts(entityIds, undefined, // topic - could be inferred from context
        ...
        const summary = this.calculateSummary(filteredSuggestions, contextWindow, input, entityIds.length);
        return {
     ...
     */
    async resolveEntityIds(entityNames) {
     ...
     * Calculate summary statistics
     */
    calculateSummary(suggestions, contextWindow, input, entityCount) {
        const suggestionTypeBreakdown = {};
...
//# sourceMappingURL=SuggestRelevantContextTool.js.map