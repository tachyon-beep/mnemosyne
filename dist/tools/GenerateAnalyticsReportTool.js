...
import { GenerateAnalyticsReportSchema } from '../types/schemas.js';
...
export class GenerateAnalyticsReportTool extends BaseTool {
        ...
        try {
            ...
            const validatedInput = withEnhancedValidation(() => {
                ...
                if (!validFormats.includes(input.format)) {
                    throw new ValidationError(`Invalid format: ${input.format}`, 'format', 'INVALID_FORMAT', 'Report format must be one of: summary, detailed, executive', [
                ...
                sections?.forEach((section, index) => {
                    if (!validSections.includes(section)) {
                        throw new ValidationError(`Invalid section: ${section}`, `sections[${index}]`, 'INVALID_SECTION', `Section '${section}' is not supported`, [
                            `Provided: ${section}`,
            ...
            const sections = await this.buildReportSections(coreReport, validatedInput.sections, validatedInput.includeRawData);
            ...
            const metadata = this.buildReportMetadata(validatedInput.timeRange, conversationCount, messageCount, generationTime, validatedInput.sections);
     ...
     */
    async generateCoreReport(timeRange, format) {
        return wrapDatabaseOperation(async () => {
            return await this.analyticsEngine.generateReport(timeRange, format);
        }, 'Failed to generate core analytics report');
    }
    /**
     * Get data counts for metadata
     */
    async getDataCounts(timeRange) {
        return wrapDatabaseOperation(async () => {
            const conversationResult = await this.conversationRepository.findByDateRange(timeRange.start, timeRange.end, 10000, 0);
     ...
     */
    async buildReportSections(coreReport, requestedSections, includeRawData) {
        const sections = [];
     ...
     */
    buildConversationMetricsSection(coreReport, includeRawData) {
     ...
     */
    buildProductivityInsightsSection(coreReport, includeRawData) {
     ...
     */
    buildKnowledgeGapsSection(coreReport, includeRawData) {
     ...
     */
    buildDecisionQualitySection(coreReport, includeRawData) {
     ...
     */
    buildRecommendationsSection(coreReport, includeRawData) {
        return {
            id: 'recommendations',
     ...
     */
    generateExecutiveSummary(coreReport, sections) {
     ...
     */
    async generateVisualizations(coreReport, sections) {
        const charts = [];
     ...
     */
    generateAppendices(coreReport, sections) {
        return [
            {
     ...
     */
    generateOverallInsights(coreReport, sections) {
        ...
        const actionItems = [
            ...
            },
            {
                ...
                priority: coreReport.decisionQuality.reversalRate > 0.2 ? 'high' : 'low',
                effort: 'medium',
                impact: 'medium',
                timeline: '2-3 weeks'
            }
     ...
     */
    buildReportMetadata(timeRange, conversationCount, messageCount, generationTime, sections) {
     ...
     */
    generateReportTitle(format, timeRange) {
        ...
        return `${formatTitles[format]} - ${startDate} to ${endDate}`;
    }
    /**
     * Get real productivity data for peak hours
     */
    async getPeakHourProductivityData(peakHours) {
        return wrapDatabaseOperation(async () => {
            const data = [];
     ...
     */
    calculateConversationProductivity(messages) {
...
//# sourceMappingURL=GenerateAnalyticsReportTool.js.map