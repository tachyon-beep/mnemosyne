 ...
 */
export class GetEntityHistoryTool extends BaseTool {
    ...
    inputSchema = GetEntityHistoryArgsSchema;
    ...
    async handle(args) {
        try {
            ...
            if (args.time_range) {
                filteredMentions = history.mentions.filter(mention => mention.createdAt >= args.time_range.start &&
            ...
            if (args.time_range) {
                filteredRelationships = history.relationships.filter(rel => rel.firstMentioned >= args.time_range.start &&
                ...
                analysis: {
                    ...
                    relationship_summary: args.include_relationships ? {
                        ...
                        strongest_relationship: filteredRelationships.length > 0
                            ? filteredRelationships.reduce((strongest, current) => current.strength > strongest.strength ? current : strongest)
                            : null
                    } : null
                },
                query_info: {
...
//# sourceMappingURL=GetEntityHistoryTool.js.map