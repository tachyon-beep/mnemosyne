...
import { DetectKnowledgeGapsSchema } from '../types/schemas.js';
import { BaseTool, wrapDatabaseOperation } from './BaseTool.js';
import { validateDateRange, validateStringArray, validateFrequency, ValidationError, formatValidationError, withEnhancedValidation } from '../utils/validation.js';
/**
 * Implementation of the detect_knowledge_gaps MCP tool
 */
export class DetectKnowledgeGapsTool extends BaseTool {
    analyticsEngine;
    ...
    async executeImpl(input, _context) {
        ...
        try {
            ...
            const validatedInput = withEnhancedValidation(() => {
                ...
                const minFrequency = validateFrequency(input.minFrequency, 'minFrequency', 1, // Minimum frequency of 1
            ...
            const { conversations, messages } = await this.getAnalysisData(validatedInput.timeRange, validatedInput.topicAreas);
     ...
     */
    async getAnalysisData(timeRange, topicAreas) {
        return wrapDatabaseOperation(async () => {
            ...
            for (const conversation of conversations) {
                ...
                if (topicAreas && topicAreas.length > 0) {
                    const filteredMessages = conversationMessages.filter(msg => topicAreas.some(topic => msg.content.toLowerCase().includes(topic.toLowerCase())));
     ...
     */
    async detectGaps(conversations, messages, validatedInput) {
        return wrapDatabaseOperation(async () => {
            const allGaps = [];
            ...
            for (const conversation of conversations) {
                ...
                const filteredGaps = gaps.filter(gap => {
                    ...
                    if (gap.frequency < validatedInput.minFrequency)
                        return false;
                    // Check if resolved gaps should be included (using frequency as proxy)
                    if (!validatedInput.includeResolved && gap.frequency <= 1)
                        return false;
                    return true;
                });
     ...
     */
    async analyzeGapCategories(knowledgeGaps) {
        return wrapDatabaseOperation(async () => {
            ...
            knowledgeGaps.forEach(gap => {
                ...
                if (gap.frequency <= 1)
                    catData.resolvedCount++; // Using frequency as proxy for resolved
            });
     ...
     */
    async analyzeTopicCoverage(conversations, messages, knowledgeGaps, topicAreas) {
        return wrapDatabaseOperation(async () => {
            ...
            return topics.map(topic => {
                const topicMessages = messages.filter(msg => msg.content.toLowerCase().includes(topic.toLowerCase()));
                const topicGaps = knowledgeGaps.filter(gap => gap.content.toLowerCase().includes(topic.toLowerCase()) ||
     ...
     */
    async analyzeGapFrequency(knowledgeGaps, timeRange) {
        return wrapDatabaseOperation(async () => {
     ...
     */
    generateResolutionSuggestions(knowledgeGaps) {
        const suggestions = [];
        knowledgeGaps
     ...
     */
    generateLearningRecommendations(knowledgeGaps, topicCoverage) {
     ...
     */
    generateInsights(knowledgeGaps, categories, frequencyAnalysis, topicCoverage) {
        const keyInsights = [];
        const concerns = [];
        const progress = [];
        const nextSteps = [];
    ...
    calculateCategoryPriority(gaps, totalFrequency, resolvedCount) {
    ...
    isMeaningfulTopicWord(word) {
        return word.length >= 4 &&
            !this.isCommonWord(word) &&
            !this.isStopWord(word) &&
            !/^\d+$/.test(word); // Not just numbers
    }
    isMeaningfulPhrase(word1, word2) {
    ...
    isTechnicalPhrase(word1, word2, word3) {
    ...
    addTopicCandidate(candidates, topic, context) {
    ...
    extractDomainSpecificTopics(sentence, candidates) {
    ...
    isTechnicalTerm(term) {
        ...
        return technicalIndicators.some(indicator => term.toLowerCase().includes(indicator));
    }
    isStopWord(word) {
    ...
    calculateCoverageDepth(messages, gaps) {
    ...
    analyzeQuestionDepth(questions) {
    ...
    calculateTopicBreadth(messages) {
    ...
    analyzeFollowUpPatterns(messages) {
        let followUpScore = 0;
    ...
    hasTopicContinuity(previous, current) {
    ...
    analyzeTechnicalDepth(messages) {
    ...
    suggestResourcesForTopic(topic, gaps) {
        const resources = [];
    ...
    async calculateTrendingGaps(gaps, timeRange) {
        if (gaps.length === 0)
            return [];
    ...
    async calculateImprovingTopics(gaps, timeRange) {
        ...
        for (const [topic, timeline] of topicTimeline.entries()) {
            ...
            const resolutionRates = timeline.periods.map(p => p.total > 0 ? p.resolved / p.total : 0);
    ...
    generateActionSuggestion(gap, type) {
        const actions = {
    ...
    generateLearningPaths(gaps, coverage) {
    ...
    generateResourceRecommendations(gaps, coverage) {
        const resources = [];
    ...
    createEmptyResponse(timeRange, input, startTime) {
        return {
            timeRange,
            analyzedAt: Date.now(),
            knowledgeGaps: [],
...
//# sourceMappingURL=DetectKnowledgeGapsTool.js.map