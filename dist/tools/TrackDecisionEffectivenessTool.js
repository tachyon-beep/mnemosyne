...
import { TrackDecisionEffectivenessSchema } from '../types/schemas.js';
import { BaseTool, wrapDatabaseOperation } from './BaseTool.js';
import { validateDateRange, validateStringArray, ValidationError, formatValidationError, withEnhancedValidation } from '../utils/validation.js';
/**
 * Implementation of the track_decision_effectiveness MCP tool
 */
export class TrackDecisionEffectivenessTool extends BaseTool {
    analyticsEngine;
    ...
    async executeImpl(input, _context) {
        ...
        try {
            ...
            const { conversations, messages } = await this.getAnalysisData(validatedInput.timeRange, validatedInput.decisionTypes);
     ...
     */
    async getAnalysisData(timeRange, decisionTypes) {
        return wrapDatabaseOperation(async () => {
            // Get conversations in time range
            const conversationsResult = await this.conversationRepository.findByDateRange(timeRange.start, timeRange.end, 1000, // Large limit for comprehensive analysis
            0);
     ...
     */
    async trackDecisions(conversations, messages, decisionTypes) {
        return wrapDatabaseOperation(async () => {
            const allDecisions = [];
     ...
     */
    async analyzeDecisionQuality(decisions, timeRange) {
        return wrapDatabaseOperation(async () => {
     ...
     */
    async analyzeDecisionOutcomes(decisions) {
        return wrapDatabaseOperation(async () => {
     ...
     */
    async analyzeDecisionReversals(decisions) {
        return wrapDatabaseOperation(async () => {
     ...
     */
    async analyzeDecisionTypes(decisions) {
        return wrapDatabaseOperation(async () => {
            ...
            const bestPerforming = byType
                ...
                .map(type => ({
                type: type.type,
                performanceScore: Math.round((type.averageQuality * 0.4) +
     ...
     */
    async analyzeDecisionProcess(decisions, conversations, messages) {
        return wrapDatabaseOperation(async () => {
     ...
     */
    generateRecommendations(decisions, qualityAnalysis, outcomeAnalysis, reversalAnalysis) {
     ...
     * Generate insights
     */
    generateInsights(decisions, qualityAnalysis, outcomeAnalysis, reversalAnalysis, typeAnalysis) {
        const keyInsights = [];
        const warnings = [];
        const positives = [];
        const concerns = [];
    ...
    analyzeQualityFactors(decisions) {
        const factorAnalysis = new Map([['Information Availability', { impacts: [], frequencies: 0 }],
    ...
    async calculateQualityTrends(decisions, timeRange) {
        if (decisions.length === 0)
            return [];
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const periodDecisions = decisions.filter(d => d.timestamp && d.timestamp >= periodStart && d.timestamp < periodEnd && d.qualityScore !== undefined);
    ...
    calculateTimeToOutcome(decisions) {
    ...
    identifySuccessFactors(decisions) {
        const successfulDecisions = decisions.filter(d => d.outcome && d.outcome >= 75 && d.qualityScore && d.qualityScore >= 70);
    ...
    incrementFactor(factorCounts, factor) {
        factorCounts.set(factor, (factorCounts.get(factor) || 0) + 1);
    }
    identifyFailurePatterns(decisions) {
        const failedDecisions = decisions.filter(d => d.outcome && d.outcome < 40 || d.qualityScore && d.qualityScore < 50);
    ...
    calculateTimeToReversal(decisions) {
        const times = decisions
    ...
    calculateReversalsByType(decisions) {
        const typeGroups = this.groupDecisionsByType(decisions);
    ...
    identifyReversalReasons(decisions) {
        const reversedDecisions = decisions.filter(d => d.reversed);
        if (reversedDecisions.length === 0)
            return [];
    ...
    calculateAverageQuality(decisions) {
    ...
    identifyTypeCharacteristics(decisions) {
        if (decisions.length === 0)
            return [];
        ...
        // Analyze consultation patterns
        const consultationRate = decisions.filter(d => d.metadata?.hasConsultation ||
            d.description?.toLowerCase().includes('consult')).length / totalDecisions;
        ...
        // Analyze data usage
        const dataUsageRate = decisions.filter(d => d.description?.toLowerCase().includes('data') ||
            d.description?.toLowerCase().includes('analysis')).length / totalDecisions;
        ...
        const structuredRate = decisions.filter(d => d.description?.toLowerCase().includes('criteria') ||
            d.description?.toLowerCase().includes('framework') ||
            d.description?.toLowerCase().includes('process')).length / totalDecisions;
        ...
        const riskAwareRate = decisions.filter(d => d.description?.toLowerCase().includes('risk') ||
            d.description?.toLowerCase().includes('impact')).length / totalDecisions;
        if (riskAwareRate > 0.4) {
            characteristics.add('Risk-conscious');
        }
        return Array.from(characteristics).slice(0, 5);
    }
    calculateAverageDecisionTime(decisions, messages) {
    ...
    analyzeInformationGathering(decisions, messages) {
        ...
        decisions.forEach(decision => {
            ...
            if (processingTime > 1800000)
                informationScore += 20; // Thorough time investment
            if (processingTime > 3600000)
                informationScore += 10; // Very thorough
            if (decision.qualityScore && decision.qualityScore > 70)
                informationScore += 15;
            if (decision.outcome && decision.outcome > 75)
                informationScore += 10;
            // Penalize for indicators of poor information gathering
            if (processingTime < 300000)
                informationScore -= 15; // Too rushed
            if (decision.reversed)
                informationScore -= 10; // May indicate insufficient info
    ...
    analyzeConsultationPatterns(decisions, messages) {
        if (decisions.length === 0) {
            return { frequency: 0, effectiveness: 0, types: [] };
        }
        let consultationCount = 0;
        let totalEffectiveness = 0;
        const consultationTypes = new Map();
        decisions.forEach(decision => {
            const description = decision.description?.toLowerCase() || '';
            ...
            if (hasConsultation) {
                ...
                if (decision.qualityScore && decision.qualityScore > 80)
                    effectivenessScore += 20;
                if (decision.outcome && decision.outcome > 80)
                    effectivenessScore += 15;
                if (decision.reversed)
                    effectivenessScore -= 20; // Consultation didn't prevent reversal
    ...
    analyzeFollowUpAdherence(decisions) {
        if (decisions.length === 0) {
    ...
    createEmptyResponse(timeRange, input, startTime, conversationCount = 0) {
        return {
            timeRange,
            analyzedAt: Date.now(),
            decisions: [],
...
//# sourceMappingURL=TrackDecisionEffectivenessTool.js.map