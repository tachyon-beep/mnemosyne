...
export class KnowledgeGraphRepository {
    ...
    findEntityByNameStmt;
    findEntitiesByTypeStmt;
    getEntityMentionsStmt;
    getEntityRelationshipsStmt;
    updateRelationshipStrengthStmt;
    constructor(db) {
        this.db = db;
    ...
    async createEntity(entity) {
        ...
        try {
            this.createEntityStmt.run(entityData.id, entityData.name, entityData.normalized_name, entityData.type, entityData.canonical_form || null, entityData.confidence_score, entityData.created_at, entityData.updated_at, JSON.stringify(entityData.metadata));
            return entityData;
        }
        catch (error) {
     ...
     */
    async findEntityByName(normalizedName) {
        try {
            const row = this.findEntityByNameStmt.get(normalizedName);
            if (!row)
                return null;
     ...
     */
    async findEntitiesByType(type, limit = 100) {
        try {
     ...
     */
    async createEntityMention(mention) {
        const now = Date.now();
        const id = this.generateId();
        const mentionData = {
            ...mention,
        ...
        try {
            this.createMentionStmt.run(mentionData.id, mentionData.entity_id, mentionData.message_id, mentionData.conversation_id, mentionData.mention_text, mentionData.start_position, mentionData.end_position, mentionData.confidence_score, mentionData.extraction_method, mentionData.created_at);
            return mentionData;
        }
        catch (error) {
     ...
     */
    async createOrUpdateRelationship(sourceEntityId, targetEntityId, relationshipType, strength, contextMessageIds = []) {
        const now = Date.now();
        // Check if relationship already exists
        const existing = this.db.prepare(`
      SELECT * FROM entity_relationships 
      WHERE source_entity_id = ? AND target_entity_id = ? AND relationship_type = ?
    `).get(sourceEntityId, targetEntityId, relationshipType);
        if (existing) {
            // Update existing relationship
            const newStrength = Math.min(1.0, existing.strength + (strength * 0.1)); // Gradual strength increase
            this.updateRelationshipStrengthStmt.run(newStrength, now, now, sourceEntityId, targetEntityId, relationshipType);
            return {
                ...existing,
            ...
            this.createRelationshipStmt.run(relationshipData.id, relationshipData.source_entity_id, relationshipData.target_entity_id, relationshipData.relationship_type, relationshipData.strength, relationshipData.first_mentioned_at, relationshipData.last_mentioned_at, relationshipData.mention_count, JSON.stringify(relationshipData.context_messages), relationshipData.created_at, relationshipData.updated_at);
     ...
     */
    async findConnectedEntities(entityId, maxDegrees = 2, minStrength = 0.3) {
        try {
            const stmt = this.db.prepare(`
      ...
      `);
            const rows = stmt.all(entityId, minStrength, maxDegrees, minStrength * 0.8);
            return rows.map(row => ({
     ...
     */
    async findShortestPath(sourceEntityId, targetEntityId) {
        try {
            const stmt = this.db.prepare(`
        ...
        LIMIT 1
      `);
            const row = stmt.get(sourceEntityId, targetEntityId);
            if (!row)
                return null;
            return {
                entity_id: targetEntityId,
                entity_name: JSON.parse(row.path_names)[JSON.parse(row.path_names).length - 1],
                entity_type: 'unknown', // Would need another query to get this
                degree: row.distance,
     ...
     */
    async findEntityClusters(minConnectionCount = 3, minAvgStrength = 0.4) {
        try {
            const stmt = this.db.prepare(`
      ...
      `);
            const rows = stmt.all(minConnectionCount, minAvgStrength);
            return rows.map(row => ({
                entity_id: row.entity_id,
                entity_name: row.entity_name,
                entity_type: row.entity_type,
     ...
     */
    async getEntityMentions(entityId, limit = 100) {
        try {
            const stmt = this.db.prepare(`
        SELECT em.*, m.content, c.title as conversation_title
        FROM entity_mentions em
    ...
    async getEntityRelationships(entityId) {
        try {
            return this.getEntityRelationshipsStmt.all(entityId, entityId);
        }
        catch (error) {
...
//# sourceMappingURL=KnowledgeGraphRepository.js.map