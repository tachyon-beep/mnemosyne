...
export class CacheRepository extends BaseRepository implements ICacheRepository {
   ...
   */
  async get(key: string): Promise<SummaryCache | null> {
    ...
    const row = this.executeStatement<{
      ...
      'find_cache_by_key',
      `SELECT id, cache_key, summary_ids, assembled_context, token_count,
   ...
   */
  async set(key: string, data: CacheSetData, _ttlHours?: number): Promise<SummaryCache> {
    if (!key || key.trim().length === 0) {
    ...
    try {
      ...
      this.handleConstraintError(error as Error, 'Cache entry');
    }
  }
  ...
  async invalidate(pattern: string): Promise<number> {
    ...
    const result = this.executeStatementRun(
      ...
      'DELETE FROM summary_cache WHERE cache_key LIKE ?',
      [pattern]
    );
  ...
  async cleanup(): Promise<number> {
    ...
    return this.transaction((_db) => {
      ...
      const expiredResult = this.executeStatementRun(
      ...
      const countResult = this.executeStatement<{ count: number }>(
        'count_cache_entries',
        'SELECT COUNT(*) as count FROM summary_cache'
      );
      ...
      if (currentCount > MAX_CACHE_ENTRIES) {
        ...
        const lruResult = this.executeStatementRun(
          'cleanup_lru_cache',
   ...
   */
  async findBySummaryId(summaryId: string): Promise<SummaryCache[]> {
   ...
   */
  async findRecentlyAccessed(limit: number = 10): Promise<SummaryCache[]> {
  ...
  async exists(key: string): Promise<boolean> {
    ...
    const result = this.executeStatement<{ count: number }>(
      'cache_exists',
      'SELECT 1 as count FROM summary_cache WHERE cache_key = ? LIMIT 1',
      [key]
    );
    return !!result;
  }
