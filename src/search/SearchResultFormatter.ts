...
export class SearchResultFormatter {
  ...
  private static readonly DEFAULT_SNIPPET_OPTIONS: Required<SnippetOptions> = {
    maxLength: 200,
    contextLength: 50,
    highlightStart: '<mark>',
    highlightEnd: '</mark>',
  ...
  private static formatSingleResult(
    ...
    const snippetInfo = this.generateEnhancedSnippet(
      message.content,
      searchTerms,
      options
    );
   ...
   */
  static generateEnhancedSnippet(
    content: string,
    searchTerms: string[],
    options?: SnippetOptions
  ): {
    ...
    // Apply highlighting
    const highlightedSnippet = this.applyHighlighting(
      snippetContent,
      searchTerms,
      bestRegion.start,
      opts
    ...
    const finalSnippet = this.addEllipsis(
      highlightedSnippet,
      bestRegion.start > 0,
      bestRegion.end < content.length,
      opts.ellipsis
    );
   ...
   */
  private static findAllMatches(content: string, searchTerms: string[]): Array<{
    start: number;
    end: number;
    term: string;
    length: number;
  }> {
    const matches: Array<{ start: number; end: number; term: string; length: number }> = [];
  ...
  private static findBestSnippetRegion(
    content: string,
    matches: Array<{ start: number; end: number; term: string; length: number }>,
    options: Required<SnippetOptions>
    ...
    for (const match of matches.slice(0, 5)) { // Limit to first few matches
      ...
      const matchesInRegion = matches.filter(m => 
        m.start >= regionStart && m.end <= regionEnd
      );
