...
import { GetProgressiveDetailSchema, GetProgressiveDetailInput } from '../types/schemas.js';
 ...
 */
export class GetProgressiveDetailTool extends BaseTool<GetProgressiveDetailInput, GetProgressiveDetailResponse> {
  ...
  protected async executeImpl(
    input: GetProgressiveDetailInput,
    _context: ToolContext
  ): Promise<GetProgressiveDetailResponse> {
    ...
    const content = await this.buildProgressiveContent(
    ...
    const navigation = await this.determineNavigation(
      currentLevel,
      tokens.used,
      maxTokens,
      conversation.id
   ...
   */
  private async buildProgressiveContent(
    conversationId: string,
    level: 'brief' | 'standard' | 'detailed' | 'full',
    maxTokens: number,
    focusMessageId?: string,
    expandContext?: boolean
  ): Promise<GetProgressiveDetailResponse['content']> {
    const content: GetProgressiveDetailResponse['content'] = {};
    let tokensUsed = 0;

    // Level 1: Brief - Just the summary
    if (level === 'brief') {
      const summary = await this.summaryRepo.findByConversationAndLevel(
        conversationId,
        'brief'
      );
      if (summary) {
    ...
    if (level === 'standard' || level === 'detailed' || level === 'full') {
      const summary = await this.summaryRepo.findByConversationAndLevel(
        conversationId,
        level === 'standard' ? 'standard' : 'detailed'
      );
      if (summary) {
        content.summary = summary.summaryText;
        tokensUsed += summary.tokenCount;
      ...
      // Add key messages
      const keyMessages = await this.selectKeyMessages(
        conversationId,
      ...
      if (keyMessages.length > 0) {
        ...
        tokensUsed += keyMessages.reduce((sum, msg) => 
          sum + this.tokenCounter.countText(msg.content).count, 0
        );
      }
    }
    ...
    if ((level === 'detailed' || level === 'full') && expandContext) {
      const contextMessages = await this.selectContextMessages(
        conversationId,
        maxTokens - tokensUsed,
        content.keyMessages?.map(m => m.id) || []
      );
    ...
    if (level === 'full') {
      const allMessages = await this.messageRepo.findByConversationId(
        conversationId,
        { limit: 1000, offset: 0 }
      );
      
      // Filter and truncate to fit token budget
      const fullMessages = this.fitMessagesToTokenBudget(
        allMessages,
        maxTokens - tokensUsed
      );
   ...
   */
  private scoreMessageImportance(message: Message, focusId?: string): number {
    let score = 0;

    // Focus message gets highest score
    if (message.id === focusId) return 1.0;

    // Questions and answers are important
    if (message.content.includes('?')) score += 0.3;
    if (message.role === 'assistant' && message.content.length > 200) score += 0.2;
  ...
  private calculateTokenUsage(
    content: GetProgressiveDetailResponse['content'],
    maxTokens: number
  ): GetProgressiveDetailResponse['tokens'] {
    let summaryTokens = 0;
    ...
    if (content.keyMessages) {
      keyMessageTokens = content.keyMessages.reduce((sum, msg) => 
        sum + this.tokenCounter.countText(msg.content).count, 0
      );
    }

    if (content.contextMessages) {
      contextTokens = content.contextMessages.reduce((sum, msg) => 
        sum + this.tokenCounter.countText(msg.content).count, 0
      );
    }
   ...
   * Determine navigation options
   */
  private async determineNavigation(
    currentLevel: 'brief' | 'standard' | 'detailed' | 'full',
    tokensUsed: number,
    maxTokens: number,
    conversationId: string
  ): Promise<GetProgressiveDetailResponse['navigation']> {
    const levelProgression: Record<string, 'standard' | 'detailed' | 'full' | undefined> = {
      'brief': 'standard',
      'standard': 'detailed',
      'detailed': 'full',
      'full': undefined
