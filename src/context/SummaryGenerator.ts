...
import { SummaryRequest } from './providers/LLMProvider.js';
...
import { SummaryValidator, ValidationResult } from './validators/SummaryValidator.js';
...
export class SummaryGenerator {
  ...
  private readonly historyRepository: SummaryHistoryRepository;
  private readonly cacheRepository: CacheRepository;
  private readonly validator: SummaryValidator;
  private readonly config: SummaryGeneratorConfig;

  constructor(
    providerManager: ProviderManager,
    summaryRepository: SummaryRepository,
    historyRepository: SummaryHistoryRepository,
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        if (this.config.enableValidation) {
          const validation = await this.validator.validateSummary(
            result.summary,
            request.messages
          );
          ...
          if (validation.score < this.config.minQualityScore) {
            throw new Error(
              `Summary quality too low: ${validation.score} < ${this.config.minQualityScore}`
            );
          }
        }
  ...
  async generateBatch(request: BatchGenerationRequest): Promise<BatchGenerationResult> {
    ...
    const failures: Array<{ request: GenerationRequest; error: Error }> = [];
    let totalCost = 0;
   ...
   */
  async invalidateConversationSummaries(conversationId: string): Promise<void> {
  ...
  private async attemptGeneration(
    ...
    // Create LLM request
    const llmRequest: SummaryRequest = {
  ...
  private determineTargetLevel(
    requestedLevel: 'brief' | 'standard' | 'detailed' | undefined,
   ...
   */
  private getConversationAge(messages: Message[]): number {
   ...
   */
  private getTemporalCategory(age: number): string {
   ...
   */
  private detectConversationType(messages: Message[]): string {
   ...
   */
  private getTimeRange(messages: Message[]): { start: Date; end: Date } | undefined {
  ...
  private async getCachedSummary(
    ...
    if (cached && Date.now() - cached.createdAt < this.config.cacheTtl) {
      try {
        return JSON.parse(cached.assembledContext) as ConversationSummary;
      } catch {
   ...
   * Start generation history tracking
   */
  private async startGenerationHistory(
    _conversationId: string,
    _level: 'brief' | 'standard' | 'detailed'
  ): Promise<string> {
   ...
   */
  private async completeGenerationHistory(
    historyId: string,
   ...
   */
  private async failGenerationHistory(historyId: string, error: Error): Promise<void> {
