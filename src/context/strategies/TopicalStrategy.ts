...
export class TopicalStrategy extends AssemblyStrategy {
  ...
  async selectItems(
    ...
    const selectedItems = this.selectFromClusters(
      rankedClusters,
      criteria,
      request
    );
   ...
   */
  private getTopicalCriteria(request: ContextAssemblyRequest): Required<StrategySelectionCriteria> {
    return {
   ...
   */
  private generateTheme(keywords: string[], _sampleContent: string): string {
  ...
  private applyTopicalOrdering(
    ...
    const itemClusterMap = new Map<string, TopicCluster>();
    ...
    const clusterGroups = new Map<string, ScoredItem[]>();
  ...
  isSuitableFor(request: ContextAssemblyRequest): boolean {
    ...
    const hasTopicalKeywords = topicalKeywords.some(keyword => 
