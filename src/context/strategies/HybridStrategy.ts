...
import { AssemblyStrategy, StrategySelectionCriteria, StrategyType } from './AssemblyStrategy.js';
...
export class HybridStrategy extends AssemblyStrategy {
  private temporalStrategy: TemporalStrategy;
  private topicalStrategy: TopicalStrategy;
  private entityStrategy: EntityCentricStrategy;
  ...
  async selectItems(
    ...
    if (dominantStrategy && dominantStrategy.weight > 0.7) {
      return this.executeSingleStrategy(
    ...
    return this.executeHybridSelection(
      relevantItems,
      request,
      budget,
      strategyWeights,
  ...
  ): Required<StrategySelectionCriteria> {
   ...
   */
  private findDominantStrategy(weights: StrategyWeight[]): StrategyWeight | null {
   ...
   * Execute hybrid selection combining multiple strategies
   */
  private async executeHybridSelection(
    items: ScoredItem[],
    request: ContextAssemblyRequest,
    budget: TokenBudget,
    strategyWeights: StrategyWeight[],
    criteria: Required<StrategySelectionCriteria>
  ): Promise<ScoredItem[]> {
    const selectedItems = new Map<string, ScoredItem>();
    ...
    for (const strategyInfo of itemsPerStrategy) {
      if (strategyInfo.itemCount === 0) continue;

      let strategyItems: ScoredItem[] = [];
   ...
   */
  private createSubBudget(budget: TokenBudget, weight: number): TokenBudget {
    return {
   ...
   */
  private calculateRecentItemsRatio(items: ScoredItem[]): number {
    ...
    const recentItems = items.filter(item => 
      now - item.createdAt < recentThreshold
    );
   ...
   */
  private calculateTopicDiversity(items: ScoredItem[]): number {
   ...
   */
  private calculateEntityDensityInQuery(query: string): number {
   ...
   */
  private calculateEntityRichnessInContent(items: ScoredItem[]): number {
    if (items.length === 0) return 0;
