...
export class TokenCounter {
  ...
  private modelConfig: typeof MODEL_CONFIGS['default'];

  constructor(config: TokenCounterConfig) {
    this.config = config;
    this.modelConfig = MODEL_CONFIGS[config.model as keyof typeof MODEL_CONFIGS] || MODEL_CONFIGS.default;
  }
   ...
   */
  calculateTokenBudget(maxTokens: number, summaryRatio: number = 0.3): {
    summaries: number;
    details: number;
    metadata: number;
    buffer: number;
   ...
   */
  private formatMessagesForSummary(messages: Message[], level: 'brief' | 'standard' | 'detailed'): string {
  ...
  updateConfig(updates: Partial<TokenCounterConfig>): void {
    ...
    if (updates.model) {
      this.modelConfig = MODEL_CONFIGS[updates.model as keyof typeof MODEL_CONFIGS] || MODEL_CONFIGS.default;
    }
  }
}
 ...
 */
export function createTokenCounter(model: string, options?: Partial<TokenCounterConfig>): TokenCounter {
 ...
 */
export function countMessageTokens(messages: Message[], model: string = 'default'): number {
  const counter = createTokenCounter(model);
 ...
 */
export function validateTokenCount(count: number, maxTokens: number, buffer: number = 100): {
  valid: boolean;
 ...
 */
export function getModelConfig(model: string): typeof MODEL_CONFIGS['default'] {
  return MODEL_CONFIGS[model as keyof typeof MODEL_CONFIGS] || MODEL_CONFIGS.default;
}