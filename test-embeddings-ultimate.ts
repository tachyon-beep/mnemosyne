/**
 * Test real embeddings work with local cache - ULTIMATE WORKING VERSION
 */
...
async function createCompleteLocalModelStructure() {
  ...
  // 1. With organization prefix (how it's cached)
  // 2. Without organization prefix (how transformers.js expects it for local models)
  const targetDirs = [
  ...
  // Verify files are accessible in both locations
  for (const targetDir of targetDirs) {
    console.log(`\n  Verifying ${targetDir}:`);
...
async function testUltimateEmbeddings() {
  console.log('🧪 Testing Ultimate Working Real Embeddings');
  ...
  try {
    ...
    for (const text of testTexts) {
      ...
      console.log(`   Debug: embedding result:`, embedding);
    ...
    console.log('\n🎉 Ultimate embeddings are functioning perfectly!');
...
// Run the test
testUltimateEmbeddings().catch(console.error);